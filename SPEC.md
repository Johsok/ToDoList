...
| **R** | **單日／週選取日切換** | 「今／週」右側提供上一日／下一日符號按鈕；今模式切換顯示日，週模式切換選取日 |
...
| 20 | **今／週旁日切換** | 新增上一日／下一日按鈕；今模式切換單日顯示，週模式切換選取日 | ✅ 新增 |
...
- 在 `週` 模式中，上一日／下一日會切換目前選取日期；若跨出目前週，切換到該日期所在週。
...
7. `今` 模式因同時段待辦採橫向排列，新增列與功能列需提高層級並解除格內裁切，確保可點擊。
...
**日切換按鈕**：「今／週」右側上一日／下一日按鈕需在手機與桌機都顯示；今模式切換單日顯示，週模式切換選取日期。
